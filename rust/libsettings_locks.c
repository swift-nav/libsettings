...
#ifdef _DEBUG
    fprintf(stderr, "%s: ts_now.tv_sec = %d, ts_now.tv_nsec = %d, ts_exp.tv_sec = %d, ts_exp.tv_nsec = %d (%s:%d)\n",
            __FUNCTION__, ts_now.tv_sec, ts_now.tv_nsec, ts_exp.tv_sec, ts_exp.tv_nsec, __FILE__, __LINE__);
#endif

    err = pthread_cond_timedwait((pthread_cond_t*)ctx->condvar, (pthread_mutex_t*)ctx->lock, &ts_exp);

#ifdef _DEBUG
    if (err != 0) fprintf(stderr, "%s: err #%d: %s, ms=%d (%s:%d)\n", __FUNCTION__, err, strerror(err), ms, __FILE__, __LINE__);
...
#ifdef _DEBUG
    fprintf(stderr, "%s: exit, res=%d (%s:%d)\n", __FUNCTION__, res, __FILE__, __LINE__);
#endif
