...
extern crate libsettings;
use libsettings::settings_manager::{lookup_setting_type, SettingType};
...
const SBP_STATE: sbp_state_t = sbp_state_t {
    state: 0, msg_type: 0, sender_id: 0, crc: 0, msg_len: 0,
    n_read: 0, msg_buff: ptr::null_mut(), io_context: ptr::null_mut(),
    sbp_msg_callbacks_head: ptr::null_mut() as *mut sbp_msg_callbacks_node_t,
    frame_buff: [0; 263], frame_len: 0
};
...
#[no_mangle]
pub extern fn r_write(buff: *mut _u8, n: _u32, ctx: *mut libc::c_void) -> _s32
{
...
#[no_mangle]
pub extern fn r_send(ctx: *mut c_void, msg_type: u16, len: u8, payload: *mut u8) -> i32
{
    let context: &mut Context = unsafe { &mut *(ctx as *mut Context) };
    unsafe {
        return sbp_send_message(&mut context.sbp_state, msg_type, SENDER_ID, len, payload, Some(r_write)) as i32;
    }
}

#[no_mangle]
pub extern fn r_send_from(ctx: *mut ::std::os::raw::c_void,
                          msg_type: u16,
                          len: u8,
                          payload: *mut u8,
                          sender_id: u16)
...
#[no_mangle]
pub extern fn r_register_cb(ctx: *mut c_void,
                            msg_type: u16,
...
#[no_mangle]
pub extern fn r_unregister_cb(ctx: *mut c_void,
                              node: *mut *mut sbp_msg_callbacks_node_t)
                              -> i32
{
    let context: &mut Context = unsafe { &mut *(ctx as *mut Context) };
    ...
    unsafe {
        if (node as i32) != 0 {
            ...
            mem::drop(Vec::from_raw_parts(*node, 0, mem::size_of::<sbp_msg_callbacks_node_t>()))
        } else {
            res = -127;
        }
    }
...
#[no_mangle]
pub extern fn r_wait(ctx: *mut c_void, timeout_ms: i32) -> i32
{
//    eprintln!("r_wait!");
    ...
    if !result { panic!("c_libsettings_wait failed"); }

    return 0;
}

#[no_mangle]
pub extern fn r_read(buff: *mut _u8, n: _u32, ctx: *mut c_void)
    -> _s32
{
...
#[no_mangle]
pub extern fn r_lock(ctx: *mut c_void)
{
//    eprintln!("r_lock");
    ...
    if !result { panic!("failed to acquire libsettings lock"); }
}

#[no_mangle]
pub extern fn r_unlock(ctx: *mut c_void)
{
//    eprintln!("r_unlock");
    let context: &mut Context = unsafe { &mut *(ctx as *mut Context) };
    let libsettings_ctx_ptr: *mut libsettings_ctx_t = &mut context.libsettings_ctx;
    ...
    if !result { panic!("failed to release libsettings lock"); }
}

#[no_mangle]
pub extern fn r_signal(ctx: *mut c_void)
{
//    eprintln!("r_signal");
    let context: &mut Context = unsafe { &mut *(ctx as *mut Context) };
    let libsettings_ctx_ptr: *mut libsettings_ctx_t = &mut context.libsettings_ctx;
    let result: bool = unsafe { c_libsettings_signal(libsettings_ctx_ptr) };
    if !result { panic!("c_libsettings_signal failed"); }
}

#[no_mangle]
pub extern fn r_wait_init(ctx: *mut c_void) -> i32
{
...
#[no_mangle]
pub extern fn r_wait_deinit(ctx: *mut c_void) -> i32
{
...
fn main()
{
    ...
    let connect_addr = cli.value_of("target").unwrap();

    eprintln!("Connecting to {}...", connect_addr);
    let the_stream = TcpStream::connect(connect_addr)
        .expect("Unable to connect to remote address");
    ...
    let c_libsettings_init_result: bool = unsafe {
        c_libsettings_init(&mut context.libsettings_ctx) };
    ...
    unsafe { sbp_state_set_io_context(&mut context.sbp_state, &mut context as *mut Context as *mut c_void) };
    ...
    if let Some(subcommand) = cli.subcommand_matches("settings-write") {

        let section_cli = subcommand.value_of("section").unwrap();
        let section = CString::new(section_cli).unwrap();

        let name_cli = subcommand.value_of("name").unwrap();
        let name = CString::new(name_cli).unwrap();

        let value_str = subcommand.value_of("value").unwrap();

        eprintln!("Writing setting: section={}, name={}, value={}", section_cli, name_cli, value_str);

        if let Some(type_) = lookup_setting_type(section_cli, name_cli) {
            let res = match type_ {
                SettingType::StInteger => {
                    let value: i32 = value_str.parse::<i32>().expect("failed to parse argument value");
                    unsafe {
                        settings_write_int(settings_ctx,
                                        ptr::null_mut(),
                                        section.as_ptr(),
                                        name.as_ptr(),
                                        value)
                ...
                SettingType::StBoolean => {
                    let value: bool = value_str.parse::<bool>().expect("failed to parse argument value");
                    unsafe {
                        settings_write_bool(settings_ctx,
                                            ptr::null_mut(),
                                            section.as_ptr(),
                                            name.as_ptr(),
                                            value)
                ...
                SettingType::StString => {
                    ...
                    unsafe {
                        settings_write_str(settings_ctx,
                                           ptr::null_mut(),
                                           section.as_ptr(),
                                           name.as_ptr(),
    ...
    context.stream.shutdown(Shutdown::Both)
