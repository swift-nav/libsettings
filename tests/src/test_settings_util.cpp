...
TEST(test_settings_util, parse) {
  ...
  char msg2[2] = {'\0', '\0'};
  EXPECT_EQ(SETTINGS_TOKENS_NAME, settings_parse(msg2, sizeof(msg2), &section, &name, &value, &type));

  char msg3[3] = {'\0', '\0', '\0'};
  EXPECT_EQ(SETTINGS_TOKENS_VALUE, settings_parse(msg3, sizeof(msg3), &section, &name, &value, &type));

  char msg4[4] = {'\0', '\0', '\0', '\0'};
  EXPECT_EQ(SETTINGS_TOKENS_TYPE, settings_parse(msg4, sizeof(msg4), &section, &name, &value, &type));
  ...
  EXPECT_EQ(SETTINGS_TOKENS_VALUE, settings_parse(sect_name_val, sizeof(sect_name_val), &section, &name, &value, &type));
  EXPECT_STREQ("sect", section);
  EXPECT_STREQ("name", name);
  ...
  EXPECT_EQ(SETTINGS_TOKENS_TYPE, settings_parse(sect_name_val_type, sizeof(sect_name_val_type), &section, &name, &value, &type));
  EXPECT_STREQ("sect", section);
  EXPECT_STREQ("name", name);
  EXPECT_STREQ("value", value);
